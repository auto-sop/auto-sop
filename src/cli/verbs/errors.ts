...
function parseSince(s: string | undefined): number | null {
  ...
  if (!m)
    throw new Error(
      `invalid --since value: ${s} (expected e.g. 30m, 24h, 7d)`,
    );
...
export function registerErrorsVerb(program: Command): void {
  program
    ...
    .option(
      '--since <duration>',
      'only show entries since duration ago (e.g. 1h, 24h, 7d)',
    )
    .option(
      '--global',
      'read from global per-project mirror (~/.claude/sop/<hash12>/errors.jsonl)',
    )
    .action(async (opts, cmd) => {
      ...
      const tail =
        typeof opts.tail === 'number' && opts.tail > 0 ? opts.tail : 20;
        ...
        if (shown.length === 0) {
          ...
          for (const e of shown) {
            ...
            process.stdout.write(
              `${pc.dim(when)} ${pc.red(e.kind ?? 'error')} ${e.msg ?? ''}\n`,
            );
          }
        }
      }
