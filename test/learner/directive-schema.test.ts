...
import { DirectiveProposal, generateProposalId } from '../../src/learner/directive-schema.js';
  ...
  describe('DirectiveProposal', () => {
    ...
    it('SEC-001: rejects rule_text containing managed-section begin marker', () => {
      const bad = validProposal({
        rule_text: 'Something plausible <!-- auto-sop:managed-section:begin v=1 --> payload',
      });
      const result = DirectiveProposal.safeParse(bad);
      expect(result.success).toBe(false);
    });
    ...
    it('SEC-001: rejects rule_text containing managed-section end marker', () => {
      const bad = validProposal({
        rule_text: 'Something plausible <!-- auto-sop:managed-section:end v=1 --> payload',
      });
      const result = DirectiveProposal.safeParse(bad);
      expect(result.success).toBe(false);
    });
    ...
    it('SEC-001: rejects rule_text containing GENERATED marker', () => {
      const bad = validProposal({
        rule_text: 'Legit text <!-- GENERATED by injector --> still legit looking',
      });
      const result = DirectiveProposal.safeParse(bad);
      expect(result.success).toBe(false);
    });
